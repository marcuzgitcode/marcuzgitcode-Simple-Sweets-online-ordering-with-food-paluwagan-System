...
<body class="animsition">
	...
	<div class="bg0 p-t-25 p-b-85">
		<div class="container">
            <div class="tab-wrap">
                <?php
                    $qrygetPaluwagan=mysqli_query($conn,"SELECT * FROM paluwagan INNER JOIN paluwagan_member ON paluwagan.paluwagan_ID=paluwagan_member.paluwagan_ID 
                    WHERE paluwagan_member.customer_ID='$data1[customer_ID]'");
                    $qrygetPaluwagan_run = mysqli_num_rows($qrygetPaluwagan);

                    $qrygetPaluwaganPending=mysqli_query($conn,"SELECT * FROM paluwagan INNER JOIN paluwagan_member ON paluwagan.paluwagan_ID=paluwagan_member.paluwagan_ID 
                    WHERE paluwagan_member.customer_ID='$data1[customer_ID]' AND paluwagan.status='0'");
                    $qrygetPaluwaganPending_run = mysqli_num_rows($qrygetPaluwaganPending);

                    $qrygetpaluwagan_memberApproved=mysqli_query($conn,"SELECT * FROM paluwagan INNER JOIN paluwagan_member ON paluwagan.paluwagan_ID=paluwagan_member.paluwagan_ID 
                    WHERE paluwagan_member.customer_ID='$data1[customer_ID]' AND paluwagan.status='1'");
                    $qrygetpaluwagan_memberApproved_run = mysqli_num_rows($qrygetpaluwagan_memberApproved);
                    
                    $qrygetpaluwagan_memberStarted=mysqli_query($conn,"SELECT * FROM paluwagan INNER JOIN paluwagan_member ON paluwagan.paluwagan_ID=paluwagan_member.paluwagan_ID 
                    WHERE paluwagan_member.customer_ID='$data1[customer_ID]' AND paluwagan.status='3'");
                    $qrygetpaluwagan_memberStarted_run = mysqli_num_rows($qrygetpaluwagan_memberStarted);

                    $qrygetOrdersCompleted=mysqli_query($conn,"SELECT * FROM paluwagan INNER JOIN paluwagan_member ON paluwagan.paluwagan_ID=paluwagan_member.paluwagan_ID 
                    WHERE paluwagan_member.customer_ID='$data1[customer_ID]' AND paluwagan.status='4'");
                    $qrygetOrdersCompleted_run = mysqli_num_rows($qrygetOrdersCompleted);

                    $qrygetOrdersCanceled=mysqli_query($conn,"SELECT * FROM paluwagan INNER JOIN paluwagan_member ON paluwagan.paluwagan_ID=paluwagan_member.paluwagan_ID 
                    WHERE paluwagan_member.customer_ID='$data1[customer_ID]' AND paluwagan.status='2'");
                    $qrygetOrdersCanceled_run = mysqli_num_rows($qrygetOrdersCanceled);
                ?>
                    <input type="radio" id="tab4" name="tabGroup2" class="tab" checked>
    				<label for="tab4">All <span>(<?= $qrygetPaluwagan_run; ?>)</span></label>
    				<input type="radio" id="tab5" name="tabGroup2" class="tab">
    				<label for="tab5">Pending <span>(<?= $qrygetPaluwaganPending_run; ?>)</span></label>
    				<input type="radio" id="tab6" name="tabGroup2" class="tab">
    				<label for="tab6">Approved<span>(<?= $qrygetpaluwagan_memberApproved_run; ?>)</span></label>
    				<input type="radio" id="tab7" name="tabGroup2" class="tab">
    				<label for="tab7">Started<span>(<?= $qrygetpaluwagan_memberStarted_run; ?>)</span></label>
    				<input type="radio" id="tab8" name="tabGroup2" class="tab">
    				<label for="tab8">Completed <span>(<?= $qrygetOrdersCompleted_run; ?>)</span></label>
    				<input type="radio" id="tab9" name="tabGroup2" class="tab">
    				<label for="tab9">Canceled <span>(<?= $qrygetOrdersCanceled_run; ?>)</span></label>
                        ...
                        <table class="table site-block-order-table mb-5">
                            ...
                            <tbody>
                                <?php
                                    ...
                                    $qrygetMyOrder=mysqli_query($conn,"SELECT * FROM paluwagan INNER JOIN paluwagan_member ON paluwagan.paluwagan_ID=paluwagan_member.paluwagan_ID 
                                    WHERE paluwagan_member.customer_ID='$data1[customer_ID]'");
                                ...
                                ?>
                                    <tr>
                                        <td class='menu__details'>
                                            <img src="admin/uploads/<?php echo $qrygetMyOrder_run['image']; ?>" alt="" class="table__image">
                                                <div class="sed">
                                                    <span>
                                                        <?php echo $qrygetMyOrder_run['paluwagan']; ?><br>
                                                    </span>
                                                </div>
                                        </td>
                                        <td>&#8369;<?php echo number_format($qrygetMyOrder_run['price'],2); ?></td>
                                        <td>&#8369;<?php echo number_format($qrygetMyOrder_run['monthly'],2); ?></td>
                                            <?php
                            ...
                            <tbody>
                                <?php
                                    ...
                                    while($qrygetMyOrder_run = mysqli_fetch_assoc($qrygetMyOrder)){
                                        ...
                                        $qrygetPaluwagan=mysqli_query($conn,"SELECT * FROM paluwagan WHERE paluwagan_ID='$qrygetMyOrder_run[paluwagan_ID]' AND status='3'");
                                        $qrygetPaluwagan_run = mysqli_fetch_assoc($qrygetPaluwagan);
                                ?>
                                    <tr>
                                        <td class='menu__details'>
                                            <img src="admin/uploads/<?php echo $qrygetPaluwagan_run['image']; ?>" alt="" class="table__image">
                                                <div class="sed">
                                                    <span>
                                                        <?php echo $qrygetPaluwagan_run['paluwagan']; ?><br>
                                                    </span>
                                                </div>
                                        </td>
                                        <td>&#8369;<?php echo number_format($qrygetPaluwagan_run['price'],2); ?></td>
                                        <td>&#8369;<?php echo number_format($qrygetPaluwagan_run['monthly'],2); ?></td>
                                            <?php
                                                if($qrygetPaluwagan_run['status']==0){
                                                    echo '<td>Pending</td>';
                                                }elseif($qrygetPaluwagan_run['status']==1){
                                                    echo '<td>Approved</td>';
                                                }elseif($qrygetPaluwagan_run['status']==2){
                                                    echo '<td>Cancelled</td>';
                                                }elseif($qrygetPaluwagan_run['status']==3){
                                                    echo '<td>Completed</td>';
                                                }else{
                                                    echo '<td></td>';
                                                }
